...
**Почему не Vercel.** У части операторов связи в Казахстане домены Vercel не открываются (соединение рвётся на TLS, `ERR_SSL_PROTOCOL_ERROR`) — проверено на живом канале. Сайт должен открываться у жюри, поэтому фронтенд разворачивается там, где это проверено: Cloudflare Pages, Netlify или статический сервис Render. Все три варианта настроены заранее, выбирать можно любой.

| Куда | Что нужно указать | Файл конфигурации |
|---|---|---|
| **Cloudflare Pages** (рекомендуется) | Root directory `frontend`, build `npm ci && npm run build`, output `dist`, переменные `VITE_API_BASE` и `NODE_VERSION=22` | `frontend/public/_redirects`, `_headers` |
...
Cloudflare Pages стоит первым, потому что у Cloudflare есть узлы в Казахстане — сайт открывается с ближайшего, а не из Европы; трафик и число сборок на бесплатном тарифе не ограничены. Netlify не хуже по удобству, но узлы дальше и бесплатных минут сборки 300 в месяц. Файл `_redirects` понимают обе платформы, поэтому переехать с одной на другую можно в любой момент.
...
Ограничения бесплатного тарифа (512 МБ памяти, половина ядра) учтены в `render.yaml`: `ONNX_THREADS=2`, `ONNX_MEM_ARENA=false`, бюджет сборки 12 секунд, меньше одновременных загрузок. Замеренное потребление памяти — 219 МБ, пик 293 МБ.
