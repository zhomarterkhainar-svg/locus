...
**Полезный профиль появляется за 3–5 секунд** (кейс разрешает 30), полная сборка с фактами, картой и описанием укладывается в 10 секунд. Повторное открытие — доли секунды.

LOCUS Startup Hackathon 2026, кейс 1 «Визуальный профиль университета», код участия `LOCUSCASE1`.

- Рабочий сайт: `ССЫЛКА НА VERCEL` (заполнить после деплоя, вид `https://candid-ai.vercel.app`)
...
Кейс разрешает 30 секунд. Сервис показывает полезный профиль за 3–5 секунд, и вот за счёт чего:
...
| **Декодирование сразу в нужном масштабе** (`draft()` у JPEG) и превью 500 px у Wikimedia | меньше трафика и в разы быстрее чтение больших кадров |
...
| Инфраструктура | Vercel (фронтенд), Render (API), Supabase, Docker, GitHub Actions (CI, обучение, keep-alive) |
...
```mermaid
flowchart LR
  U[Браузер, Vercel] -- поиск --> S[/api/search/]
  S --> WD[(Wikidata)]
...
frontend/vercel.json    настройки развёртывания фронтенда
...
## Развёртывание: Vercel + Render + Supabase

Схема: фронтенд на Vercel (статика, CDN), API с моделями на Render, общий кэш и отметки в Supabase. Всё в бесплатных тарифах.
...
### 2. Render — API (10 минут)

1. [render.com](https://render.com) → **New → Blueprint** → подключить GitHub-репозиторий. Render прочитает `render.yaml` и создаст сервис `candid-ai-api`.
2. В **Environment** задать секреты: `GEMINI_API_KEY`, при желании `FLICKR_API_KEY`, `SUPABASE_URL`, `SUPABASE_KEY`, `CONTACT` (ссылка на репозиторий — этого требуют правила Wikimedia и OSM).
3. Дождаться первой сборки (5–8 минут: ставятся зависимости и качаются веса ONNX).
4. Проверить `https://<имя>.onrender.com/api/health` — должно быть `"status": "ok"` и `models.clip = "ready"`.

Бесплатный инстанс Render засыпает после 15 минут без запросов; первый запрос после сна занимает около минуты. Workflow **Keep backend awake** будит его раз в 10 минут — задайте переменную репозитория `RENDER_URL`. Это честно отражено в интерфейсе: пока сервис просыпается, показывается соответствующее сообщение.

Ограничения бесплатного тарифа (512 МБ памяти, половина ядра) учтены в `render.yaml`: `ONNX_THREADS=2`, `ONNX_MEM_ARENA=false`, бюджет сборки 12 секунд, меньше одновременных загрузок.

### 3. Vercel — фронтенд (5 минут)

1. [vercel.com](https://vercel.com) → **Add New → Project** → тот же репозиторий.
2. **Root Directory**: `frontend`. Остальное Vercel возьмёт из `frontend/vercel.json` (Vite, `npm run build`, SPA-переадресация).
3. **Environment Variables**: `VITE_API_BASE` = адрес сервиса Render, например `https://candid-ai-api.onrender.com`.
4. **Deploy**. После сборки скопировать домен и дописать его в переменную `CORS_ORIGINS` на Render (через запятую), затем **Manual Deploy → Restart**.

Превью-развёртывания Vercel (`*.vercel.app`) разрешены в CORS отдельным правилом, их не нужно перечислять вручную.

### 4. Альтернатива: один контейнер
...
**Внешние сервисы.** Wikidata API, Wikimedia Commons API, Overpass API (четыре зеркала параллельно), OSRM FOSSGIS, Open-Meteo, Wikipedia REST API, Flickr API, Google Gemini API, CARTO basemaps, Supabase, сайты вузов, GitHub Actions, Vercel, Render.
